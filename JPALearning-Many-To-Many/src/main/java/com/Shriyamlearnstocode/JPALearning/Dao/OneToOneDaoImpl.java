...
//note:in onetomany/many to many where default fetchtype is set to lazy and we want to find/fetch the entity
//like here instuctorman and its related entity together then we use join fetch bcz join fetch fetches the
//entity and its related entity even if fetchtype is set to lazy
	...
	public Course findCourseAndStudentByCourseId(int theId) {
		//note:in onetomany/many to many where default fetchtype is set to lazy and we want to find/fetch the entity
		//like here course and its related entity together then we use join fetch bcz join fetch fetches the
		//entity and its related entity even if fetchtype is set to lazy
		Course cs= entityManager.createQuery("From Course C Join Fetch C.students Where C.id= :data", Course.class)
